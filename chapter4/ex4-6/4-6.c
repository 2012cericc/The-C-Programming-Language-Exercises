...
double vars[26];    // NEW: each index corresponds to a letter
int vp = 0;         // NEW: next free variable position
    ...
    while((type = getop(s)) != EOF){
        switch(type){
            case NUMBER:
                push(atof(s));
                break;
            case VARIABLE:      // NEW: variable was used, push if valid
                if((s[0]-'a')<vp && vp<26){
                    op2 = vars[s[0]-'a'];
                    push(op2);
                }else{
            ...
            case '/':
                ...
                break;
            case '\n':      // NEW: modified to save value to variable array
                op2 = pop();
                if(vp < 26){
                    vars[vp] = op2;
                    printf("\t%c = %.8g\n", ('a'+vp), op2);
...
int getop(char s[]){
    ...
    s[1] = '\0';
    if(!isdigit(c) && c != '.'){
        if(isalpha(c)){     // encounter variable, error check in main
            s[0] = c;
            return VARIABLE;
        }else {
            return c;
